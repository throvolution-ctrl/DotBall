...
playTone(330, "sine", 0.12, 0.1, 0.08);
playTone(440, "triangle", 0.1, 0.18, 0.15);
},
...
musicOsc.frequency.linearRampToValueAtTime(
Math.min(base, 180),
c.currentTime + 0.18
);
...
normalHit(colorIdx) {
playTone(200 + colorIdx * 40, "triangle", 0.14, 0.08);
},

perfectHit(colorIdx) {
const freq = 320 + colorIdx * 55;
playTone(freq, "sine", 0.18, 0.1);
playTone(freq * 1.5, "triangle", 0.1, 0.08, 0.05);
},
...
playTone(base * 1.25, "sine", 0.1, 0.12, 0.1);
},

perfectSpectrum() {
[330, 440, 550, 660].forEach((f, i) =>
playTone(f, "sine", 0.16, 0.18, i * 0.1)
);
},

submitScore() {
playTone(440, "sine", 0.12, 0.12);
playTone(660, "triangle", 0.1, 0.16, 0.08);
},
...
node.style.transform = `translate3d(${trail.x - BALL_R}px, ${
trail.y - BALL_R
}px, 0) scale(${1 - t * 0.08})`;
}
}
...
const speedScale = baseBall.speedScale ?? gs.current.speedScale ?? 1;

return {
id: 0,
x: baseBall.x,
y: baseBall.y,
vx: angle.vx * speedScale,
vy: angle.vy * speedScale,
prevX: baseBall.x,
prevY: baseBall.y,
...
(g, ball, hitOffset, isForgiven = false) => {
...
showMsg(
`PERFECT ×${g.perfectChain === 0 ? PERFECT_CHAIN_MAX : g.perfectChain}`
);
}
...
const nearRight =
ball.x > paddleRight && ball.x <= paddleRight + PADDLE_FORGIVENESS_PX;
const fastEnough = Math.abs(ball.vx) >= MIN_VELOCITY_FOR_FORGIVENESS;

const forgiven =
!standard && wasInside && nowOutside && fastEnough && (nearLeft || nearRight);

if (standard || forgiven) {
candidates.push({
index: i,
forgiven,
distance: Math.abs(ball.x - g.paddleX),
});
}
}
...
INITIAL_SPEED *
(g.speedScale ?? 1) *
(ball.id === 2 ? 3.0 : ball.id === 1 ? 2.6 : 2.2);
...
if (ballEl) {
ballEl.style.transform = `translate3d(${ball.x - BALL_R}px, ${
ball.y - BALL_R
}px, 0)`;
}
}
...
paddleRef.current.style.transform = `translate3d(${
g.paddleX - g.paddleW / 2
}px, 0, 0)`;
}
...
const speedScale = Math.max(1, Math.min(1.25, arenaW / 520));
const g = gs.current;
...
g.paddleX = Math.max(
g.paddleW / 2 + 2,
Math.min(g.arenaW - g.paddleW / 2 - 2, rawX)
);
}, []);
...
if (paddleRef.current) {
paddleRef.current.style.width = `${g.paddleW}px`;
}
}
});
...
style={{
color: currentColor,
textShadow: `0 0 16px ${currentShadow}`,
}}
>
{score}
</span>
...
style={{
color: currentColor,
textShadow: `0 0 10px ${currentShadow}`,
}}
>
...
ref={(el) => {
if (!trailRefs.current[ballIndex]) {
trailRefs.current[ballIndex] = [];
}
...
<button className="db-btn" onClick={startGame}>
START
</button>
</div>
)}
...
<button className="db-btn" onClick={togglePause}>
RESUME
</button>
</div>
)}
...
className={`db-lb-row${
index === newEntryIdx ? " db-lb-row--new" : ""
}`}
>
